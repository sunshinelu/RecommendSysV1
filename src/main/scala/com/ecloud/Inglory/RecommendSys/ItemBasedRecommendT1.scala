 ...
 * 代码运行失败！
 */
object ItemBasedRecommendT1 extends Serializable {
  def main(args: Array[String]) {
    //bulid environment
    val spark = SparkSession.builder.appName("ItemBasedRecommendT1").getOrCreate()
    ...
    //work
    val itemRecomm = rdd_app_R8.map { f => (f._1, f._2, f._3) }
    val itemRecomDF = itemRecomm.toDF("userid", "itemid", "pref")

    //val itemRecomm = rdd_app_R8.map{ f => UserRecomm(f._1, f._2, f._3)}

    //val itemRecomDF = spark.createDataFrame(itemRecom).toDF()
    //val itemRecomDF = itemRecomm.toDF()
