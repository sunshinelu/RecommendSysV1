 ...
 * 读取t_yhxw_log_prep表中数据构建基于物品的协同过滤模型，并将结果保存到mysql数据库中
 */
object ItemBasedRecommendT extends Serializable {
  def main(args: Array[String]) {
    //bulid environment
    val spark = SparkSession.builder.appName("ItemBasedRecommendT").getOrCreate()
