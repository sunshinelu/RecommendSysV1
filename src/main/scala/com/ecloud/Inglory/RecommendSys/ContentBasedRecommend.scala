 ...
 * 计算yeeso-test-ywk_webpage表中文档之间的相似性。
 *
 *
 */
object ContentBasedRecommend {
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("ContentBasedRecommend")
    val sc = new SparkContext(sparkConf)
    //    val sqlContext = new SQLContext(sc)
    val sqlContext = new HiveContext(sc)
    //load stopwords file
    val hdfs = "hdfs://192.168.37.23:8020/personal/sunlu/lulu/yeeso/Stopwords.dic"
    ...
    joinDF2.rdd.map(row => (row(2), row(3), row(5), row(6), row(7), row(8), row(10), row(11))).
      map { x => {
        ...
        //put.add(Bytes.toBytes("info"), Bytes.toBytes("urlKey"), Bytes.toBytes(x._7.toString))
