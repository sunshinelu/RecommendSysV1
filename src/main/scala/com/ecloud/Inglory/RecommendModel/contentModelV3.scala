...
import org.apache.hadoop.hbase.client.{ResultScanner, Put, HBaseAdmin, Scan}
 ...
 * contentModelV3与contentModel2的区别在于：
 *
 * 读取ylzx_xgwz的的info:simsScore列，对相似性打分值进行过滤
 *
 *
 */
object contentModelV3 {
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName(s"contentModelV3") //.setMaster("local[*]").set("spark.executor.memory", "2g")
 ...
 */
    ...
    val df6 = df5.withColumn("rn", row_number.over(w)).where($"rn" <= 70)//70
