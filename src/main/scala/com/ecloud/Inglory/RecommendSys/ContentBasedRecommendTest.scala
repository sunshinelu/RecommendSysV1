 ...
 * 读取HBASE中数据，并计算文本之间的相似性，最终将结果保存到solr中。
 */
object ContentBasedRecommendTest {
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("ContentBasedRecommendMethod2")
    ...
    scan.addColumn(Bytes.toBytes("p"), Bytes.toBytes("manuallabel"))
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
    ...
    }.filter(x => null != x._2 & null != x._3 & null != x._4 & null != x._6).
      map { x => {
      ...
      }.filter(x => x._3.length > 0 & x._4.length > 0 & x._6.length > 0)
...
/*
    joinDF2.rdd.map(row => (row(2), row(3), row(5), row(6), row(7), row(8), row(10), row(11))).
      map { x => {
        val paste = x._7 + "::score=" + x._2.toString
        ...
        put.add(Bytes.toBytes("info"), Bytes.toBytes("title"), Bytes.toBytes(x._3.toString))
...
*/
    joinDF2.rdd.map(row => (row(2), row(3), row(5), row(6), row(7), row(8), row(10), row(11))).
      map(x => {
        val id = x._7 + "::score=" + x._2.toString
        val label = x._5.toString
        val level = x._2.toString
        val relateUrl = x._4.toString
        val simsScore = x._1.toString
        val time = x._6.toString
        val url = x._8.toString
        val title = x._3.toString

        val solrUrl_similarArticleRec = "http://192.168.37.11:8983/solr/solr-yilan-similarArticleRec";
        //val server = new HttpSolrServer(solrUrl_similarArticleRec)
        val server = new HttpSolrClient(solrUrl_similarArticleRec)
        ...
        val response:UpdateResponse = server.add(document)
        server.commit()
      })
