 ...
 * 运行成功！（但是，标签列中文乱码！！！！！！尚未解决）
 */
object alsDataProcessedV2 {
  ...
  def main(args: Array[String]) {
    //bulid environment
    val spark = SparkSession.builder.appName("alsDataProcessedV2").getOrCreate()
    ...
    //val df3 = df2.filter(df2.col("PARAMS").contains("id="))
    //val df4 = df2.filter(df2("REQUEST_URI").contains("search/getContentById.do")||df2("REQUEST_URI").contains("like/add.do")||df2("REQUEST_URI").contains("searchByKeyword.do"))
    ...
    val hbaseDate = hBaseRDD.map { case (k, v) => {
      ...
      (key, title, label, time)
    }
    }.filter(x => null != x._1 ).//URL列不为空
      map { x => {
        val rowkey_1 = Bytes.toString(x._1)
        val title_1 = Bytes.toString(x._2)
         val label_1 = if(null != x._3)  Bytes.toString(x._4) else "null"
        //时间格式转化
        val time_1 = if(null != x._4){
          ...
          temp } else "null"
        (rowkey_1, title_1, label_1, time_1)
      }
    ...
    val hbaseDateRowRDD = hbaseDate.map { f => Row(f._1, f._2, f._3, f._4) }
    //RowRDD to DF
    val yeesoDateDF = spark.createDataFrame(hbaseDateRowRDD, schema3)
    val joinDF3 = joinDF2.join(yeesoDateDF, Seq("itemString"), "left")
    ...
    val joinDF5 = joinDF4.select("userString", "itemString", "rating", "rn", "title", "label", "time")
    /*
    //代码出错
        val pasteFunc = udf((userString: String, rn: Integer) =>
        {userString + "::score=" + rn})

        val joinDF5 = joinDF4.select("userString", "itemString","rating","rn","title","label","time").
            withColumn("rowkey", pasteFunc(col("userString"), col("rn")))
        ...
        //将结果保存到数据框中
        joinDF5.write.mode("overwrite").jdbc(url2, "als_Test", prop2)
    */
    joinDF5.rdd.map(row => (row(0), row(1), row(2), row(3), row(4), row(5), row(6))).
      map(x => {
        ...
        val label = if (null != x._6) x._6.toString else ""
        val time = if (null != x._7) x._7.toString else ""
        (userString, itemString, rating2, rn, title, label, time)
      }).
      map { x => {
