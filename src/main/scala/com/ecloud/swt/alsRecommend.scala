...
/**
 * Created by sunlu on 17/6/6.
 *
 * SPEC_LOG_CLICK(第三方系统访问日志记录表):
  LOG_ID: 唯一标识（不使用）
  OPERATOR_ID: 用户ID
  USERNAME: 用户名
  USERIP: 用户IP(不使用)
...
object alsRecommend {
  ...
  def getYlzxRDD(ylzxTable: String, sc: SparkContext): RDD[ylzxSchema] = {
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      x.title.length >= 2
    }).filter(x => x.time > 0).filter(x => x.manuallabel.contains("商务"))
  ...
  def getLogsRDD(logsTable: String, spark: SparkSession, sc: SparkContext): RDD[logsSchema] = {
    ...
    val rdd2 = rdd1.map(x => {
      ...
      val rating = x.value match {
        ...
        case x if (x < UtilTool.getOneYear()) => 0.3 * x
        case _ => 0.0
      }

      logsSchema(x.operatorId, x.userString, x.itemString, x.accessTime,x.accessTimeL,rating)
    })//.filter(_.operatorId == "7a4e4f92-7357-4057-8b39-7f5f96f341c2")
  ...
  def main(args: Array[String]) {
    ...
    val spark = SparkSession.builder().appName("shangwuting: alsRecommend").getOrCreate()
