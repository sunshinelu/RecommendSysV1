 ...
 * 读取HBASE中的数据，计算文本之间的相似性，并将全部的相似性数据保存到HBASE中
 *
 */
object docSims {
  def main(args: Array[String]) {
    ...
    //bulid environment
    val spark = SparkSession.builder.appName("docSims").getOrCreate()
    ...
    //对dataframe进行分组排序，并取每组的前500个
    //计算两个向量的余弦相似度，值越大就表示越相似。
    val w = Window.partitionBy("doc1").orderBy(col("sims").desc)
    val dfTop5 = docSimsDF.withColumn("rn",row_number.over(w)).where(col("rn") <= 100)
