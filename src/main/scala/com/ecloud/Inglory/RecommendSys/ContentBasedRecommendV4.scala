 ...
 * 获取近5年的数据进行分析
 * 使用提取的关键词计算文章的相似性，每篇文章提取20个关键词
 */
object ContentBasedRecommendV4 {
  def main(args: Array[String]) {
    ...
    //bulid environment
    val spark = SparkSession.builder.appName("ContentBasedRecommendV4-document similarity").getOrCreate()
    ...
    val wordsUrlLab = hbaseRDD.map { x =>
      ...
      val time = x._1._5
      val webName = x._1._6
      (keywords, id, urlID, title, label, time, webName) //关键词、id、urlID、标题、label、time、websitename
    })
    ...
    val schema3 = StructType(
      StructField("doc1", StringType) ::
        ...
        :: Nil)
    val docSimsRowRDD = sim_threshhold.map { x => {
