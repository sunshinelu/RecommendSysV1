...
object getKeyWordsV2 {
  def main(args: Array[String]) {
    ...
    //bulid environment
    val spark = SparkSession.builder.appName("getKeyWordsV2").getOrCreate()
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      map { x => {
        ...
        (rowkey_1, title_1, content_1, label_1)
      }
      }.filter(x => x._2.length > 0 & x._3.length > 0).filter(_._4.contains(keywords)).
      map { x => {
    ...
    //防止路径下该文件夹存在
    val filepath = "/personal/sunlu/lulu/yeeso/getKeyWords_" + keywords
