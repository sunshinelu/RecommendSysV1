 ...
 * 在DailyReading进行标准化处理时，对权重进行标准化处理，取值范围为［0，1］。
 * 由于权重分布跨度较大，导致标准化处理＋保留三位有效数字之后很多值为0.
 * 因此更改策略，不使用［0，1］均一化处理，对weight取log值。
 */
object DailyReadingV2 {
...
*/
    ...
    */
    ...
    val keywordsDF2 = keywordsDF.withColumn("vScaled", bround(log($"v"), 3)).
